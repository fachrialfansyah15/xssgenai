...
class BaseCrawler(ABC):
    def __init__(
        ...
        self._base_hostname = (parsed_start.hostname or "").lower()
        self._base_netloc = (parsed_start.netloc or "").lower()
        try:
            ip_address(self._base_hostname)
            self._base_is_ip = True
        except (AddressValueError, ValueError):
            self._base_is_ip = False
        self.lock = Lock()
    ...
    def _is_out_of_scope(self, url: str) -> bool:
        ...
        if not host or not self._base_hostname:
            return True

        if self._base_is_ip:
            return host != self._base_hostname

        if host == self._base_hostname:
            return False

        return not host.endswith(f".{self._base_hostname}")
