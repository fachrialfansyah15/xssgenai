...
def dynamic_dom_inspect(url: str, timeout: int = 30000, hash_fuzz: bool = True, fuzz_payloads: List[str] | None = None) -> List[Dict]:
    ...
    # Quick DNS/connectivity pre-check to avoid noisy Playwright errors
    try:
        ...
        try:
            ...
            logger.warning(f"dynamic_dom_inspect DNS not resolvable for {url}")
            return []
    except Exception:
        pass
    ...
    for attempt in range(max_attempts):
        try:
            ...
            last_error = exc
            logger.warning(f"dynamic_dom_inspect Playwright attempt {attempt + 1} failed for {url}: {exc}")
            time.sleep(2)
        except Exception as exc:
            last_error = exc
            logger.warning(f"dynamic_dom_inspect unexpected failure for {url}: {exc}")
            break

    if not playwright_success:
        logger.info(f"dynamic_dom_inspect falling back to requests-only mode for {url}")
        return _requests_only_dom(url)
...
def _requests_only_dom(url: str) -> List[Dict]:
    try:
        resp = make_request(url)
        if resp and resp.text:
            snippet = (resp.text or '')[:2000]
            return [{"type": "html_snapshot", "detail": snippet, "frame": url}]
    except Exception as exc:
        logger.debug(f"requests-only fallback failed for {url}: {exc}")
    return []
