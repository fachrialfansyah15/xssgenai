...
try:
    from fastreflect_worker import get_worker as _fast_get_worker, shutdown as _fast_shutdown  # type: ignore
except Exception:
    _fast_get_worker = None
    _fast_shutdown = None
else:
    if _fast_shutdown is not None:
        atexit.register(_fast_shutdown)

# cache state for fast worker
...
def _flatten_json(value: Any, prefix: str = "", out: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Flatten nested JSON dictionaries into dotted-key mapping.
    Lists are treated as opaque values to avoid lossy reconstruction.
    """
    if out is None:
        out = {}
...
def _set_nested_value(target: Dict[str, Any], dotted_key: str, value: Any) -> None:
    """
    Set value inside nested dictionary following dotted key path.
    """
    if not dotted_key:
        return
...
def parse_raw_http_request(raw: str, default_scheme: Optional[str] = None) -> Dict[str, Any]:
    """
    Parse raw HTTP request (as exported from a proxy) and return structured data.
    Raises ValueError when the payload cannot be parsed.
    """
    if not raw or not raw.strip():
        raise ValueError("Raw request kosong.")
    ...
    while lines and not lines[0].strip():
        lines.pop(0)
    if not lines:
        raise ValueError("Tidak menemukan request line.")

    request_line = lines.pop(0).strip()
    try:
        ...
        raise ValueError(f"Request line tidak valid: {request_line}") from exc

    # Header parsing (supports simple folded headers)
    ...
    if target.startswith(("http://", "https://")):
        ...
        scheme = parsed_target.scheme or scheme
        host = parsed_target.netloc or host
        ...
        if not host:
            raise ValueError("Header Host tidak ditemukan; tidak dapat membangun URL absolut.")
        host = host.strip()
        if "://" in host:
            parsed_host = urlparse(host if host.startswith(("http://", "https://")) else f"{scheme}://{host}")
            scheme = parsed_host.scheme or scheme
            host = parsed_host.netloc or parsed_host.path or host
    ...
    for key, values in query_params.items():
        base_template[key] = values[0] if values else ""
        param_locations[key] = "query"
        ...
        if "application/json" in content_type:
            try:
                ...
                if isinstance(json_body, dict):
                    json_flat = _flatten_json(json_body)
                    for key, value in json_flat.items():
                        if key:
                            base_template[key] = value
                            param_locations[key] = "json"
                    body_type = "json"
                else:
                    body_type = "raw"
            except Exception:
...
def build_surfaces_from_raw_request(parsed_request: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Convert parsed raw HTTP request into parameter surfaces compatible with XSSTester.
    Each surface retains baseline template and metadata for replaying the original request.
    """
    base_template = dict(parsed_request.get("base_template") or {})
    if not base_template:
        return []

    request_meta = {
        "headers": dict(parsed_request.get("headers") or {}),
        "query_params": copy.deepcopy(parsed_request.get("query_params") or {}),
        "form_params": copy.deepcopy(parsed_request.get("form_params") or {}),
        "json_body": copy.deepcopy(parsed_request.get("json_body") or {}),
        "json_flat": dict(parsed_request.get("json_flat") or {}),
        "body_type": parsed_request.get("body_type"),
        "body": parsed_request.get("body"),
        "param_locations": dict(parsed_request.get("param_locations") or {}),
        "base_template": base_template,
    }

    is_form = parsed_request.get("body_type") == "form"
    method = parsed_request.get("method", "GET").upper()
    url = parsed_request.get("url")

    surfaces: List[Dict[str, Any]] = []
    for name in base_template.keys():
        entry = {
            "url": url,
            "method": method,
            "data_template": dict(base_template),
            "is_form": is_form,
            "name": name,
            "request_meta": copy.deepcopy(request_meta),
            "source": "raw_request",
        }
...
def _get_fast_worker():
    global _FAST_DISABLED
    if _FAST_DISABLED or _fast_get_worker is None:
        return None
    worker = _fast_get_worker()
    if worker is None:
        _FAST_DISABLED = True
    return worker


def _disable_fast_worker() -> None:
    global _FAST_DISABLED
    _FAST_DISABLED = True


def _decode_all_python(text: str) -> str:
    r"""
...
def decode_all(text: str) -> str:
    if not text:
        return ""
    worker = _get_fast_worker()
    if worker:
        try:
            return worker.decode(text)
        except Exception:
            _disable_fast_worker()
    return _decode_all_python(text)
...
def contains_unescaped(haystack: str, needle: str) -> bool:
    ...
    if worker:
        try:
            return worker.contains(haystack, needle)
        except Exception:
            _disable_fast_worker()
    decoded = _decode_all_python(haystack)
...
) -> Tuple[str, Optional[Dict[str, str]], Optional[Any]]:
    """
    Persiapkan URL, header tambahan, dan body untuk request.
    Jika request_meta tersedia (misal dari raw request), gunakan sebagai baseline saat
    merekonstruksi query/body sehingga parameter lain tetap ikut terkirim.
    """
    parsed = urlparse(base_url)
    method = method.upper()

    if request_meta:
        headers = dict(request_meta.get("headers") or {})
        for key in ("Host", "host", "Content-Length", "content-length"):
            headers.pop(key, None)

        param_locations = request_meta.get("param_locations") or {}
        query_params_src = request_meta.get("query_params") or {}
        form_params_src = request_meta.get("form_params") or {}
        json_flat_src = request_meta.get("json_flat") or {}
        json_body_src = request_meta.get("json_body") or {}
        body_type = request_meta.get("body_type")

        query_params = {k: list(v) for k, v in query_params_src.items()}
        form_params = {k: list(v) for k, v in form_params_src.items()}
        json_flat = dict(json_flat_src)
        ...
        for key, value in (data or {}).items():
            ...
            if loc == "query":
                ...
                form_params[key] = [value] if not isinstance(value, list) else value
            elif loc == "json":
                json_flat[key] = value
            else:
        ...
        final_url = parsed._replace(query=qs).geturl()

        body_payload: Optional[Any] = None
        if body_type == "json":
            json_body = copy.deepcopy(json_body_src) if isinstance(json_body_src, dict) else {}
            for dotted_key, val in json_flat.items():
                if dotted_key:
                    _set_nested_value(json_body, dotted_key, val)
            ...
            for key, values in form_params.items():
                if isinstance(values, list):
                    if len(values) == 1:
                        form_payload[key] = values[0]
                    else:
                        form_payload[key] = values
                else:
                    form_payload[key] = values
            body_payload = form_payload
        elif body_type == "raw":
            body_payload = request_meta.get("body")
        else:
            body_payload = None

        return final_url, headers or None, body_payload
...
def fetch_dynamic_html(url: str, wait_selector: str = None, timeout: int = 15000) -> Optional[str]:
    ...
    """
    # If running inside an asyncio event loop, avoid Playwright sync API and
    # fallback to plain HTTP fetch via requests/session to prevent runtime errors.
    try:
        import asyncio as _asyncio
        try:
            _asyncio.get_running_loop()
            try:
                resp = session.get(url, timeout=(timeout/1000.0 if timeout else None)) if session else None
            except Exception:
                resp = None
            return (resp.text if getattr(resp, 'text', None) else "") if resp else ""
        except RuntimeError:
            pass
    except Exception:
        pass
