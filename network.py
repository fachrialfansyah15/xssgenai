...
except Exception:  # pragma: no cover - urllib3 always present alongside requests but guard anyway
...
def ensure_insecure_warning_suppressed() -> None:
    global _INSECURE_WARNING_SUPPRESSED
    if _INSECURE_WARNING_SUPPRESSED:
        return
...
try:
    ...
    DEFAULT_VERIFY = certifi.where()
except Exception:
    DEFAULT_VERIFY = True  # fallback to system trust store
...
def make_request(
    ...
    if not url or not isinstance(url, str):
        logger.error("Request skipped: URL kosong atau bukan string (%r)", url)
        return None

    url = url.strip()
    parsed_url = urlparse(url)
    if parsed_url.scheme.lower() not in {"http", "https"} or not parsed_url.netloc:
        logger.warning("Request skipped: URL tidak valid atau tanpa skema HTTP/HTTPS (%s)", url)
        return None
    ...
    try:
        ...
        # Graceful fallback on SSL certificate issues: retry once with verify=False
        try:
            from requests.exceptions import SSLError  # type: ignore
        except Exception:
            SSLError = tuple()  # type: ignore

        msg = str(exc) if exc else ""
        is_ssl_err = False
        try:
            is_ssl_err = isinstance(exc, SSLError) or ("CERTIFICATE_VERIFY_FAILED" in msg)
        except Exception:
            is_ssl_err = "CERTIFICATE_VERIFY_FAILED" in msg

        if is_ssl_err and session.verify is not False:
            try:
                ensure_insecure_warning_suppressed()
                logger.warning(f"SSL verification failed for {url}; retrying insecure (verify=False).")
                response = session.request(
                    method=method.upper(),
                    url=url,
                    params=params,
                ...
                logger.error(f"Request to {url} failed after insecure retry: {exc2}")
                return None
